  ...
  if (code) {
    ...
    if (next.startsWith("/portal")) {
      ...
      if (user) {
        ...
        //
        // This route used to hand back a session and redirect, leaving status
        // to the page guards. That was survivable while nothing linked here —
        // recovery, invite and email change each have their own route. Google
        // OAuth makes this the main entry door, and a pending or suspended user
        // would otherwise walk away holding a live session cookie: bounced from
        // pages, but still carrying credentials every app/api/** handler has to
        // keep refusing. Refuse once, at the door, and drop the session.
        //
        // Scoped to portal destinations, so non-portal callbacks are unaffected.
        ...
        if (!profile) {
          // handle_new_user provisions on insert into auth.users, so a missing
          // row means that trigger did not run. Fail closed rather than admit
          // an identity no profile governs.
        ...
        if (profile.status !== "approved") {
          // Allowlist, matching requireUser: an unrecognised status is a
          // refusal, not a pass.
          await supabase.auth.signOut();
          return NextResponse.redirect(new URL("/access-denied", url.origin));
        }
        ...
        // The same bookkeeping a password sign-in performs, so an OAuth login
        // is not invisible in the audit trail.
        await Promise.all([
          (async () => {
            try {
              const svc = await createServiceClient();
