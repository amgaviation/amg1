...
export const metadata = { title: "Edit Invoice - Crew Portal" };
...
export default async function CrewEditInvoicePage({
  params,
  searchParams,
}: {
  ...
  if (!detail || detail.invoice.submitter_id !== user.id) {
    redirect("/portal/crew/invoices?error=not-found");
  }
  if (!vendorInvoiceEditable(detail!.invoice.status)) {
    redirect(`/portal/crew/invoices?error=locked&record=${id}`);
  }
  const missions = await listMissionOptionsForContractor(user.id, "crew");
  ...
  return (
    <>
      ...
      <VendorInvoiceForm
        ...
        backTo={`/portal/crew/invoices/${id}/edit`}
        invoiceId={id}
