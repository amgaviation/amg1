...
function SubmissionDetails({ row }: { row: FormSubmission }) {
  ...
  return (
    <details className="rounded-lg border border-white/10 bg-white/[0.04]">
      ...
      <div className="grid gap-6 border-t border-white/10 p-4 lg:grid-cols-2">
        ...
        <div className="lg:col-span-2">
          <h3 className="text-sm font-semibold text-white">Message / Summary</h3>
          <p className="mt-2 whitespace-pre-wrap rounded-lg border border-white/10 bg-[#050B14]/55 p-4 text-sm leading-6 text-slate-200">
            {row.message || row.requested_support_summary || "-"}
          </p>
        </div>
        {conditional.length ? (
          <div className="lg:col-span-2">
            <h3 className="text-sm font-semibold text-white">Conditional Details</h3>
            <dl className="mt-2 rounded-lg border border-white/10 bg-[#050B14]/55 p-4">
        ...
        {payload.length ? (
          <div className="lg:col-span-2">
            <h3 className="text-sm font-semibold text-white">All Submitted Fields</h3>
            <dl className="mt-2 rounded-lg border border-white/10 bg-[#050B14]/55 p-4">
        ...
        <form action={updateFormSubmission} className="grid gap-3 rounded-lg border border-white/10 bg-[#050B14]/55 p-4 lg:col-span-2">
          <input type="hidden" name="id" value={row.id} />
          <label className="grid gap-2 text-sm font-semibold text-slate-100">
            Submission Status
            <select name="status" defaultValue={row.status} className="h-11 rounded-md border border-white/12 bg-[#07111F] px-3 text-sm text-white">
          ...
          <label className="grid gap-2 text-sm font-semibold text-slate-100">
            Admin Notes
            <textarea name="admin_notes" defaultValue={row.admin_notes ?? ""} className="min-h-24 rounded-md border border-white/12 bg-[#07111F] px-3 py-2 text-sm text-white" />
          </label>
    ...
    <PortalShell role="admin" user={user}>
      ...
      <SectionCard title="Filters" icon="clipboard">
        <form className="grid gap-3 md:grid-cols-[1fr_1fr_2fr_auto] md:items-end">
          ...
          <label className="grid gap-2 text-sm font-semibold text-slate-100">
            Status
            <select name="status" defaultValue={params.status || "All"} className="h-11 rounded-md border border-white/12 bg-[#050B14] px-3 text-sm text-white">
              <option>All</option>
              {submissionStatuses.map((status) => (
                <option key={status} value={status}>{statusLabel(status)}</option>
              ))}
          ...
          <label className="grid gap-2 text-sm font-semibold text-slate-100">
            Search
            <input name="q" defaultValue={params.q || ""} className="h-11 rounded-md border border-white/12 bg-[#050B14] px-3 text-sm text-white placeholder:text-slate-500" placeholder="Name, email, company, tail, aircraft, path" />
          </label>
      ...
      <SectionCard title="Website Inquiries" icon="clipboard">
        {submissions.length === 0 ? (
          ...
          <div className="space-y-3">
            {submissions.map((row) => (
              <article key={row.id} className="rounded-lg border border-white/10 bg-white/[0.04] p-4">
                <div className="grid gap-3 xl:grid-cols-[1.2fr_1fr_1fr_auto] xl:items-start">
                  <div>
                    <p className="text-sm font-semibold text-white">{row.requester_name || row.full_name}</p>
                    <p className="mt-1 text-xs text-slate-400">{row.email} | {row.phone}</p>
                    <p className="mt-1 text-xs text-slate-400">{display(row.company || row.organization || row.company_operator)}</p>
                  </div>
                  <div>
                    <p className="text-xs uppercase text-slate-500">{row.source_page}</p>
                    <p className="mt-1 text-sm font-semibold text-white">{display(row.support_type ?? row.support_path ?? row.service_interest ?? row.inquiry_type)}</p>
                  </div>
                  <div>
                    <p className="text-xs text-slate-400">{display(row.aircraft || row.aircraft_type)} {row.tail_number ? `| ${row.tail_number}` : ""}</p>
                    <p className="mt-1 text-xs text-slate-400">{display(row.timing || row.timeline_urgency)}</p>
                    <p className="mt-1 text-xs text-slate-400">{formatDateTime(row.created_at)}</p>
                  </div>
                  <div className="flex flex-wrap gap-2 xl:justify-end">
                    <span className="rounded-full border border-white/10 bg-white/[0.06] px-3 py-1 text-xs font-semibold text-slate-200">{statusLabel(row.status)}</span>
                    <span className={`rounded-full border px-3 py-1 text-xs font-semibold ${row.email_sent ? "border-emerald-500/30 bg-emerald-500/10 text-emerald-100" : "border-amber-500/30 bg-amber-500/10 text-amber-100"}`}>
