  ...
  return (
    <PortalShell role="admin" user={user}>
      <PageHeader
        ...
        actions={
          <Link href="/portal/admin/messages" className="text-xs text-muted-foreground hover:text-accent">
            Back to messages
          </Link>
        }
      />
      ...
      <SectionCard icon="messageSquare">
        ...
        <form action={postMessage} className="mt-6 flex gap-3 border-t border-border pt-5">
          ...
          <input type="hidden" name="back_to" value="/portal/admin/messages" />
          <textarea
            name="body"
            required
            placeholder="Type your reply..."
            className="flex-1 rounded-lg border border-input bg-background px-3 py-2 text-sm outline-none focus:border-accent"
            rows={3}
          />
