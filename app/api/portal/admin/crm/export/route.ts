...
export async function GET(request: Request) {
  // Role alone is not enough: getSessionUser resolves any valid JWT carrying a
  // portal role, and the status filtering lives in requireUser — a page guard
  // that never runs for app/api/**. Without the status check a suspended admin
  // keeps exporting the entire pipeline until their token expires. Every
  // sibling admin route already uses this helper.
