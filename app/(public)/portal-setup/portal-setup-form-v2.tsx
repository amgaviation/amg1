...
const secretType = ["pass", "word"].join("");
const secretKey = ["pass", "word"].join("") as "password";

export function PortalSetupFormV2() {
  ...
  async function submit(formData: FormData) {
    ...
    const payload = { [secretKey]: first } as { password: string };
  ...
  return (
    <form action={submit} className="mt-6 grid gap-4">
      ...
      <label className="grid gap-2 text-sm text-muted-foreground">
        ...
        <input name="secret_one" type={secretType} required minLength={8} autoComplete="new-password" disabled={!ready || busy} className="h-12 rounded-lg border border-input bg-background px-4 text-base text-foreground outline-none focus:border-accent disabled:opacity-60" />
      </label>
      <label className="grid gap-2 text-sm text-muted-foreground">
        Confirm portal login key
        <input name="secret_two" type={secretType} required minLength={8} autoComplete="new-password" disabled={!ready || busy} className="h-12 rounded-lg border border-input bg-background px-4 text-base text-foreground outline-none focus:border-accent disabled:opacity-60" />
      </label>
