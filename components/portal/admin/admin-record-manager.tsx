...
export function AdminRecordManager({
  ...
  const editorModal = editor ? (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/45 p-4" role="dialog" aria-modal="true">
      <div className="max-h-[92vh] w-full max-w-4xl overflow-hidden rounded-lg border border-slate-200 bg-white shadow-2xl">
  ...
  if (selected) {
    const detailGroups = selected.detailSections?.length ? selected.detailSections : [{ title: "Details", rows: selected.details }];
    ...
    return (
      <section className="space-y-5">
        <button
          ...
          className="inline-flex min-h-10 items-center rounded-md border border-slate-200 bg-white px-3 text-sm font-semibold text-slate-700 shadow-sm transition-colors hover:border-primary/40 hover:bg-blue-50"
        >
          Back to {title}
        </button>

        <div className="rounded-lg border border-slate-200 bg-white shadow-[0_14px_36px_rgba(15,23,42,0.06)]">
          <header className="border-b border-slate-200 bg-slate-50/80 px-5 py-5">
            <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
              <div className="min-w-0">
                <p className="text-[0.62rem] font-bold uppercase tracking-[0.18em] text-primary">Record Detail</p>
                <h2 className="mt-2 break-words font-display text-3xl font-extrabold uppercase leading-none text-slate-950">
                  {selected.title}
                </h2>
                {selected.subtitle ? <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">{selected.subtitle}</p> : null}
              ...
              <div className="flex flex-wrap items-center gap-2">
                <Button type="button" variant="outline" className="rounded-full" onClick={() => startRefresh(() => router.refresh())} disabled={isRefreshing}>
            ...
            {selected.details.length ? (
              <dl className="mt-5 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
                {selected.details.slice(0, 4).map((detail) => (
                  <div key={`meta-${detail.label}`} className="rounded-md border border-slate-200 bg-white px-3 py-2">
                    <dt className="text-[0.62rem] font-bold uppercase tracking-[0.14em] text-slate-500">{detail.label}</dt>
                    <dd className="mt-1 min-w-0 break-words text-sm font-semibold text-slate-950">{valueText(detail.value)}</dd>
                  </div>
                ))}
              </dl>
            ) : null}
            ...
            <div className="grid gap-5 md:grid-cols-2">
              {detailGroups.map((section) => (
                <section key={section.title} className="rounded-lg border border-slate-200 bg-white p-4">
                  <h3 className="text-[0.66rem] font-bold uppercase tracking-[0.16em] text-slate-500">{section.title}</h3>
                  <dl className="mt-3">
            ...
            <aside className="space-y-5">
              {selected.tabs?.length ? (
                selected.tabs.map((tab) => (
                  <section key={tab.title} className="rounded-lg border border-slate-200 bg-slate-50/70 p-4">
                    <h3 className="text-[0.66rem] font-bold uppercase tracking-[0.16em] text-slate-500">{tab.title}</h3>
                    {tab.rows?.length ? (
                      <dl className="mt-3">
                        {tab.rows.map((item) => (
                          <div key={`${tab.title}-${item.label}`} className="grid gap-1 border-b border-slate-200/80 py-2 last:border-0">
                            <dt className="text-xs font-semibold text-slate-700">{item.label}</dt>
                            <dd className="min-w-0 break-words text-sm text-slate-600">{valueText(item.value)}</dd>
                          </div>
                        ))}
                      </dl>
                    ) : (
                ...
                <section className="rounded-lg border border-dashed border-slate-300 bg-slate-50/80 p-5 text-sm text-slate-500">
