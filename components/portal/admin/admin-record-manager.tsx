...
export function AdminRecordManager({
  ...
  return (
    <section className="w-full max-w-full overflow-hidden rounded-lg border border-slate-200 bg-white text-slate-950 shadow-[0_18px_44px_rgba(8,20,36,0.07)]">
            ...
            {activeFilterBadges.map(([key, value]) => {
              ...
              return (
                <button
                  ...
                  className="inline-flex items-center gap-2 rounded-full border border-primary/30 bg-blue-50 px-3 py-1 text-xs font-semibold text-primary transition-colors hover:border-primary/60"
                >
      ...
      <div className="flex flex-col gap-2 border-b border-slate-200 bg-slate-50/70 px-5 py-3 text-xs text-slate-500 sm:flex-row sm:items-center sm:justify-between">
        <p>
                ...
                <tbody className="bg-white">
                  {pagedRows.map((row) => (
                    <tr
                      ...
                      className={cn(
                        "cursor-pointer border-b border-slate-100 bg-white outline-none transition-colors hover:bg-slate-50 focus-visible:bg-blue-50 focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-primary/60",
                      ...
                      {columns.map((column, index) => (
                        <td key={column.key} className={cn("bg-inherit px-4 py-3 align-middle text-slate-700", column.className)}>
                          {index === 0 ? (
                            ...
                            <span className="block truncate text-slate-700" title={valueText(row.cells[column.key])}>{valueText(row.cells[column.key])}</span>
                          )}
                        </td>
                      ))}
            ...
            <div className="grid gap-3 p-4 md:hidden">
              {pagedRows.map((row) => (
                <button
                  ...
                  type="button"
                  onClick={() => openRecord(row)}
                  className="rounded-lg border border-slate-200 bg-white p-4 text-left shadow-sm outline-none transition-colors hover:border-primary/45 focus-visible:ring-2 focus-visible:ring-primary/60"
                >
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      <p className="truncate font-semibold text-slate-950">{row.title}</p>
                      <p className="mt-1 truncate text-sm text-slate-500">{valueText(row.cells.email)}</p>
                    </div>
      ...
      {visibleRows.length > pageSize ? (
        <div className="flex flex-col gap-3 border-t border-slate-200 bg-slate-50/70 px-5 py-4 text-sm text-slate-500 sm:flex-row sm:items-center sm:justify-between">
          <span>
      ...
      {selected ? (
        <div className="fixed inset-0 z-40 flex justify-end bg-slate-950/45 backdrop-blur-sm" role="dialog" aria-modal="true" onClick={() => setSelectedId("")}>
          <aside
            className="h-full w-full overflow-y-auto border-l border-slate-200 bg-white text-slate-950 shadow-2xl sm:max-w-[42rem]"
      ...
      {editor ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/45 p-4 backdrop-blur-sm" role="dialog" aria-modal="true">
          <div className="max-h-[92vh] w-full max-w-4xl overflow-hidden rounded-lg border border-slate-200 bg-white text-slate-950 shadow-2xl">
