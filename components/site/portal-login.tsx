...
import { signIn, signInWithGoogle, signUp } from "@/app/portal/actions/auth";
...
const ERROR_MESSAGES: Record<string, string> = {
  ...
  oauth:
    "Google sign-in is unavailable right now. Use your email and password, or contact AMG Operations.",
};
...
/**
 * Google's four-colour mark, inlined. The CSP forbids remote images, and a
 * self-hosted copy would be one more asset to keep in step with Google's
 * branding requirements — which do require the official mark on this button.
 */
function GoogleMark() {
  return (
...
/**
 * One button for both tabs: signing in with Google and requesting access with
 * Google are the same handshake. A first-time Google identity is provisioned
 * pending_approval by the handle_new_user trigger and stopped at
 * /auth/callback, so this never skips the approval queue — it only replaces the
 * password.
 */
function GoogleAuthButton({ isSignIn }: { isSignIn: boolean }) {
  return (
    <div className="mb-6">
      <form action={signInWithGoogle}>
        ...
        <button
          type="submit"
          className="inline-flex min-h-12 w-full items-center justify-center gap-3 rounded-full bg-white px-6 py-4 text-sm font-medium text-[#1f1f1f] transition hover:bg-white/90"
        >
          <GoogleMark />
      ...
      <div className="mt-5 flex items-center gap-3" aria-hidden="true">
        ...
        <span className="font-mono text-[10px] uppercase [letter-spacing:0.18em] text-[var(--oc-aluminum-2)]">
          or {isSignIn ? "sign in with email" : "request access by email"}
        </span>
        <span className="h-px flex-1 bg-white/[0.10]" />
      </div>
    </div>
