...
export function MovementIntro() {
  ...
  const windowScale = useTransform(scrollYProgress, [0, 0.6], [1, 1.8]);
  const windowY = useTransform(scrollYProgress, [0, 0.6], [0, -300]);
  const skyScale = useTransform(scrollYProgress, [0, 0.6], [1, 1.18]);
  const skyY = useTransform(scrollYProgress, [0, 0.6], ["0%", "-10%"]);
  const indicatorOpacity = useTransform(scrollYProgress, [0, 0.26], [1, 0]);
  const heroOpacity = useTransform(scrollYProgress, [0.8, 1], [1, 0]);
  const heroPointerEvents = useTransform(heroOpacity, (opacity) => (opacity < 0.05 ? "none" : "auto"));
  ...
  const animatedHero = reduceMotion ? {} : { opacity: heroOpacity, pointerEvents: heroPointerEvents };

  return (
    <div ref={wrapperRef} className="relative h-[200vh] bg-[var(--background)]">
      <motion.section
        style={animatedHero}
        className="sticky top-0 isolate h-[100vh] w-full overflow-hidden bg-[var(--background)] text-[var(--foreground)]"
