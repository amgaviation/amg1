...
const {
  ...
  parseCsv,
  parseFlightTime,
} = require("./import-crew-profiles-from-csv");
...
test("normalizeCrewCsvRow maps CSV fields, booleans, numbers, and import metadata", () => {
  const row = Object.fromEntries(CSV_HEADERS.map((header: string) => [header, ""]));
  row["First Name"] = " Stan ";
  row["Last Name"] = " Jones ";
...
test("normalizeCrewCsvRow skips rows with no name and flags missing contact data", () => {
  const blank = Object.fromEntries(CSV_HEADERS.map((header: string) => [header, ""]));
