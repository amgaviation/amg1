...
- Do not add Chain of Thought to reasoning-native models (o3, o4-mini, DeepSeek-R1, Qwen3 thinking mode, Claude Fable 5) — they think internally, CoT degrades output
- Never instruct Claude Fable 5 / Mythos 5 to reveal or explain its internal reasoning — this triggers refusal classifiers
...
**Claude (claude.ai, Claude API, Claude 4.x / Claude 5)**

The Claude 5 family (**Fable 5** / **Mythos 5**) sits above Opus. **Opus 4.8** remains the default on many surfaces. If the user says "Fable", "Mythos", "Claude 5", or describes long-running autonomous / multi-day agent work, route to the Fable 5 section below. Otherwise assume Opus 4.8 unless a version is named.
...
**Claude Fable 5 / Mythos 5** (Anthropic's Mythos-class tier, above Opus)

Built for problems too complex, long-running, or ambiguous for prior models — multi-day autonomous runs, first-shot correctness on well-specified complex work, dense-image vision, code review, subagent orchestration. Give it the hardest version of the task; simple workloads undersell it. Source: Anthropic's official "Prompting Claude Fable 5" guide.

*Core prompting rules:*
- **Effort, not thinking budgets** — effort is the primary intelligence/latency/cost lever. Default `high`; `xhigh` for the most capability-sensitive work; `medium`/`low` for routine tasks (still strong — often beats prior models' best). Never add "think step by step" or thinking-budget instructions.
- **Prevent overplanning** — add: "When you have enough information to act, act. Do not re-derive established facts, re-litigate decided questions, or narrate options you will not pursue. If weighing a choice, give a recommendation, not a survey."
- **Prevent over-engineering at high effort** — add: "Don't add features, refactor, or introduce abstractions beyond what the task requires. Do the simplest thing that works well. Only validate at system boundaries."
- **Brief steering beats enumeration** — instruction following is strong enough that one short instruction replaces listing every unwanted behavior. E.g. brevity: "Lead with the outcome — first sentence answers 'what happened.' Be selective about what you include rather than compressing into fragments or arrow chains."
...
- **State the boundaries** — it can take unrequested actions. Add: "When the user is describing a problem or thinking out loud, the deliverable is your assessment — report findings and stop. Don't apply a fix until asked."
- **NEVER instruct it to reveal, echo, or explain its internal reasoning** — this can trigger a `reasoning_extraction` refusal and fall back to Opus 4.8. Strip "explain your reasoning" / "show your thinking" lines from prompts being adapted for Fable 5.
- Not intended for offensive cybersecurity or biology/life-sciences work — such requests can return refusals; note this if the user's task touches those domains.

*For long autonomous / agentic runs, add as needed:*
- **Ground progress claims**: "Before reporting progress, audit each claim against a tool result from this session. Only report work you can point to evidence for; if unverified, say so."
- **Checkpoint discipline**: "Pause for the user only when the work genuinely requires them: a destructive or irreversible action, a real scope change, or input only they can provide."
- **Anti-early-stopping (autonomous pipelines)**: "You are operating autonomously; the user cannot answer mid-task. For reversible actions that follow from the original request, proceed without asking. Before ending your turn, if your last paragraph is a plan, question, or promise of undone work — do that work now."
- **Subagents**: it delegates well — "Delegate independent subtasks to subagents and keep working while they run." Fresh-context verifier subagents beat self-critique: "Verify your work with subagents against the specification every [interval]."
- **Memory**: for repeated runs, give it a notes file: "Store one lesson per file with a one-line summary at the top. Record corrections and confirmed approaches alike, including why they mattered."
- **Context reassurance** (if the harness shows token counts): "You have ample context remaining. Do not stop, summarize, or suggest a new session on account of context limits."
- **Final-summary readability**: "Your final summary is for a reader who didn't see the work. Open with the outcome, drop working shorthand, write complete sentences, spell out terms."

*When adapting an existing prompt or skill for Fable 5:* older prompts are often too prescriptive and degrade output — strip rigid step-by-step scaffolding, reasoning-echo instructions, and enumerated behavior lists; keep intent, constraints, and acceptance criteria.
...
### Full-Stack Web Stack Modifiers (Supabase / Vercel / Next.js and similar)

When the target is a coding agent (Claude Code, Cursor, Cline, Devin, Bolt/v0/Lovable, etc.) AND the project involves a managed backend + hosting stack (Supabase, Vercel, Next.js, or similar), layer these constraints into the generated prompt — the database and production deploys are one bad agent action away:

- **Database is a review gate, always**: "Stop and ask before creating or editing any migration, RLS policy, database trigger, or edge function. Propose the SQL/diff first." Schema and RLS mistakes are silent security holes, not visible bugs.
- **RLS explicitly**: any prompt touching auth or user data should require "every new table gets an RLS policy; state who can read/write each table and why."
- **Deploy verification, not assumption**: use preview deployments as the checkpoint — "Verify on the Vercel preview deployment before reporting done; production deploys require my approval."
- **Env vars by name, never by value**: reference `SUPABASE_SERVICE_ROLE_KEY`, `NEXT_PUBLIC_*` etc. as names only; flag that service-role keys must never reach client-side code (`NEXT_PUBLIC_` prefix = shipped to the browser).
- **Integration scope**: for third-party integrations (payments, email, webhooks), require sandbox/test mode by default and a named list of which external services the agent may call.
- **Client/server boundary**: state which logic belongs in server components / API routes / edge functions vs. the client — generators default to putting too much in the client.
...
- CoT added to o3/o4-mini/R1/Qwen3-thinking/Fable 5 → REMOVE IT
- "Explain your reasoning" aimed at Fable 5 → REMOVE IT (refusal trigger)
